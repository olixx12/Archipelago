...
def load_regions(player : int, multiworld : MultiWorld):
    ...
    region_list = json.load(region_file)
...
def load_tables(item_name_to_id,location_name_to_id):
    ...
    location_file = pkgutil.get_data(__name__,"locations.json").decode("utf-8")
    locations = json.load(location_file)
    for category_name,category in locations.items():
        if category_name == "Level":
            for i in range(2,category["last_index"] - category["first_index"] + 2):
                location_name_to_id[f"Level {i}"] = (category["first_index"]+i) - 2
        else:
            item_name_to_id.update(category)
